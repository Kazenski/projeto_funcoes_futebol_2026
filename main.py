...
from equipe.clube import criar_clube, contratar_jogador, calcular_forca_total
from equipe.logPartida import registrar_lance, exportar_sumula_txt
...
def iniciar_jogo():
    ...
    if mercado:
        ...
        for i, jog in enumerate(mercado):
            print(
                f"[{i}] {jog['nome']} (+{jog['bonus_atk']} ATK, +{jog['bonus_def']} DEF)")
    ...
    print("\n" + "=" * 50)
    print(
        f"APITA O ÁRBITRO! COMEÇA O JOGO: {meu_time['nome']} x {rival['nome']}")
    print("=" * 50)
    ...
    for lance in range(1, 6):
        ...
        print(
            f"📊 PLACAR: {meu_time['nome']} {meu_time['GOLS']} x {rival['GOLS']} {rival['nome']}")
    ...
    exportar_sumula_txt(meu_time['nome'], rival['nome'],
                        meu_time['GOLS'], rival['GOLS'])
